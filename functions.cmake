...
function(vespa_add_target_dependency TARGET OTHER_TARGET)
    ...
    set_property(GLOBAL APPEND PROPERTY TARGET_${OTHER_TARGET}_TARGET_DEPENDENTS ${TARGET})
...
function(__create_module_source_dependencies)
    ...
    foreach(MODULE IN LISTS VESPA_MODULES)
        ...
        if(TARGETS)
            ...
            unset(TARGET_DEPENDENTS)
            unset(MODULE_DEPENDENTS)
            foreach(TARGET IN LISTS TARGETS)
                get_property(TARGET_TARGET_DEPENDENTS GLOBAL PROPERTY TARGET_${TARGET}_TARGET_DEPENDENTS)
                get_property(TARGET_MODULE_DEPENDENTS GLOBAL PROPERTY TARGET_${TARGET}_MODULE_DEPENDENTS)
                list(APPEND TARGET_DEPENDENTS ${TARGET_TARGET_DEPENDENTS})
                list(APPEND MODULE_DEPENDENTS ${TARGET_MODULE_DEPENDENTS})
                add_dependencies(${TARGET} ${SOURCE_TARGET})
            endforeach()
            if (TARGET_DEPENDENTS)
                list(REMOVE_DUPLICATES TARGET_DEPENDENTS)
                foreach(TARGET_DEPENDENT IN LISTS TARGET_DEPENDENTS)
                    add_dependencies(${TARGET_DEPENDENT} ${SOURCE_TARGET})
                endforeach()
            endif()
